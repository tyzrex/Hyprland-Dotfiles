...
choice=$(echo -e "$options" | rofi -dmenu -replace -config ~/dotfiles/rofi/config-tyzrex-launcher.rasi -l 5) 

case $choice in
	$option1)
        $HOME/dotfiles/apps/tyzrex_Dotfiles_Settings-x86_64.AppImage ;;
	$option2)
        $HOME/dotfiles/apps/tyzrex_Hyprland_Settings-x86_64.AppImage ;;
	$option3)
